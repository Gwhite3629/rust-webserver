...
pub fn default_accept_encoding<'req>(
    val: String,
...
pub fn default_authorization<'req>(
    val: String,
    state: &'req mut RequestState,
) -> RequestEffect<'req> {
    // Receive basic auth
    ...
    if val.contains("Basic") {
        ...
        RequestEffect::DECODER(Some(Box::new(move |a| {
    ...
    // Client response should equal this value
        ...
        let pairs: Vec<(&str, &str)> = client_vals
            .iter()
        ...
        RequestEffect::DECODER(Some(Box::new(move |a| {
            if realm != a.realm.as_str() {
                ...
                let ha1: String = format!(
                    "{:x}",
                    md5::compute(a.user.clone() + ":" + a.realm.as_str() + ":" + a.pass.as_str())
                );
                let ha2: String = format!(
                    "{:x}",
                    md5::compute(
                        unsafe { state.auth.method.as_str() }.to_owned()
                            + ":"
                            + &unsafe { state.auth.uri.to_string() }
                    )
                );
                // Response = MD5(HA1:servernonce:nc:cnonce:qop:HA2)
...
pub fn default_connection<'req>(
    _val: String,
    _state: &'req mut RequestState,
) -> RequestEffect<'req> {
    todo!()
}

pub fn default_content_length<'req>(
    _val: String,
    _state: &'req mut RequestState,
) -> RequestEffect<'req> {
    todo!()
